...
proc sql noprint;
    ...
    execute(
        ...
        *where prv_no ne 0;
        create table t20 as select * from connection to db2(select * from session.&user_id._x20);
    disconnect from db2;
...
data r140p.obssitevis;
    set reportdata;
    status=compress(sts_cd||sts_r_cd);
run;

proc means data= r140p.obssitevis nway noprint;
    class state county status;
    output out=obscounts(drop=_freq_ _type_) n=count;
run;

proc transpose data=obscounts out=r140p.obssitevis_counts(drop=_name_ _label_);
    by state county;
    id status;
run;
